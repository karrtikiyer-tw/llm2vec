...
class ModifiedGemma2DecoderLayer(Gemma2DecoderLayer):
    def __init__(self, config: Gemma2Config, layer_idx: int):
        ...
        # Override only the attention module with our modified version
...
class Gemma2BiModel(Gemma2Model):
    ...
    def __init__(self, config: Gemma2Config):
        if not is_transformers_attn_greater_or_equal_4_41():
            raise ValueError(
                "The current implementation of Gemma2BiModel follows modeling_gemma2.py of transformers version >= 4.41.0"
            )
        
        super().__init__(config)
        
        # Override layers with our modified bidirectional layers
...
class Gemma2BiForMNTP(Gemma2ForCausalLM):
    def __init__(self, config):
        ...
        # Override model with our bidirectional version
